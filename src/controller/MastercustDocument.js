...
// Updated controller with only search functionality
exports.getCustomersBySearch = async (req, res) => {
    try {
        ...
        const searchField = req.query.searchField || 'partyUID'; // default search by Party UID
        const offset = (page - 1) * limit;
        ...
        // Add single field search filter
        if (searchValue && searchField) {
            ...
            switch (searchField) {
                ...
                case 'mobile':
                    ...
                    break;
                default:
                    fieldName = 'id'; // default to Party UID
            }

            baseQuery += ` AND ${fieldName} LIKE ?`;
            countQuery += ` AND ${fieldName} LIKE ?`;
